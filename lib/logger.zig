...
pub fn print(allocator: std.mem.Allocator, comptime format: []const u8, args: anytype) void {
    if (is_wasm) {
        const buf = std.fmt.allocPrint(allocator, format, args) catch unreachable;
